...
Тгбот на Go для скачивания аудиодорожек из YouTube, принимает ссылку на видео и отправляет файлом в .m4a. Использует `yt-dlp` для скачивания и `ffmpeg` для его работы 
...
### Качество 
на ютубе аудио хранится в `.opus`, он эффективнее чем необходимый нам `.m4a` (.opus тг воспринимает как странные головые в отличии от .m4a), при конвертации ffmpeg старается сохранить размер файла и Kbps, изза чего качество страдает. это ограничение можно убрать в `config.toml` значением `song_quality`:
- `song_quality = "default"` - файл остается исходного размера **( ~192 Kbps, 2 - 4 Mb )**
- `song_quality = "max"` - сохраняет максимум информации **( 400+ Kbps, 6 - 10 Mb )**

настройки (сборка команды для скачивания) `yt-dlp` находятся в [ytdlp.go](./internal/downloader/ytdlp.go)
